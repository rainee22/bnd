<section id="hero" style="background-image: url(../images/backgrounds/wave-bg-6.svg)" class="bg-cover">
    <div class="container-fluid container-lg px-5 pt-md-3 pb-md-5 text-body-emphasis" style="margin-top:50px">
        <div class="row gx-5 align-items-center justify-content-center" data-aos="fade-up">
            <div class="col-md-8 col-lg-6" data-aos="zoom-in">
                <div class="my-5 text-center text-xl-start">
                    <h1 class="display-6 fw-bolder mb-2" id="typing-animation">
                        
                    </h1>
                    <p class="lead fw-normal mb-4">
                        We offer a choice of system development services
                        to meet your demands. For your business, we provide the best and most personalized solutions.
                    </p>


                    <div class="d-grid gap-2 d-sm-flex justify-content-sm-center justify-content-xl-start">
                        <a class="btn btn-primary btn-lg px-4 me-1 shadow-sm rounded-pill" href="{{route('contact')}}"
                            data-aos="slide-right" data-aos-delay="200">
                            Get Started
                        </a>
                        <a class="btn btn-secondary btn-lg px-4 shadow rounded-pill" href="{{route('about-us')}}"
                            data-aos="slide-right" data-aos-delay="100">
                            Learn More
                        </a>
                    </div>
                </div>
            </div>

            <div class="col-md-9 col-lg-6 d-none d-md-block text-center" data-aos="zoom-out">
                <img class="img-fluid animated-up-down" src="{{ asset('images/software-developer.svg') }}"
                    alt="Software Developer" loading="lazy" />
            </div>
        </div>
    </div>
    <br><br><br><br><br><br>
    <p style="text-align: center;">Trusted by individuals and teams at the world's
        best companies</p>
    <script>
        var text = "We provide high-quality IT solution services at a reasonable cost.";
        var speed = 40; // Adjust the typing speed here (in milliseconds)

        function typeWriter() {
            var element = document.getElementById("typing-animation");
            var i = 0;
            var timer = setInterval(function() {
                if (i < text.length) {
                    element.innerHTML += text.charAt(i);
                    i++;
                } else {
                    clearInterval(timer);
                }
            }, speed);
        }

        typeWriter();
    </script>
    @livewire('main.components.partners')
</section>
